...
<p>
totalPage = <span>${pageMaker.totalPage}</span>&nbsp&nbsp&nbsp&nbsp 
scri.(next)bidx = <span>${pageMaker.scri.bidx}</span>
</p>
...
<!--  
<form name="form1" action = "${pageContext.request.contextPath}/Spring/MVCListcontroller_TS${pageMaker.totalMakeSearch(pageMaker.scri.page)}" method = "get">
-->
...
<td class=col></td>
     ...
     <c:if test="${list != null }">
	     ...
	     <tr>
	        ...
	        <a title="미리보기" href="${pageContext.request.contextPath}/Spring/MVCContentcontroller${pageMaker.bidxIdxTotalMakeSearch(pageMaker.scri.page, bv.bidx, mv.idx, mv.name)}">
	                                                ${bv.subject}</a></td>
	  ...
	  <c:if test="${list == null }">
		  ...
		  <script type='text/javascript'>alert('데이터가 없습니다.\n\n처음부터 시작합니다.');	  
	  	  window.document.location.href='${pageContext.request.contextPath}/Spring/MVCWritecontroller${pageMaker.idxSend(mv.idx, mv.name)}';</script>
