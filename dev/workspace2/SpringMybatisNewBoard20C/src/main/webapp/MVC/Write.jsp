...
//    form0922.action="${pageContext.request.contextPath}/Spring/MVCWrite_Actioncontroller?page=${page}&totalPage=${pageMaker.totalPage}&scri=${scri}"; 
	form0922.action="${pageContext.request.contextPath}/Spring/MVCWrite_Actioncontroller${pageMaker.idxTotalMakeSearch(page, mv.idx, mv.name)}";
...
<p>pageMaker.tatalPage = ${pageMaker.totalPage}</p> 
<p>scri.searchType = ${scri.searchType}</p> 
<p>scri.keyword = ${scri.keyword}</p> 
${mv.idx} / ${mv.name}님 반갑습니다.

<!--  
	<c:if test="${bv.idx == null}">
	  <c:set var="idx" value="${'7'}"/>	
	</c:if>
-->

     <!-- InsertDB()가 실패하면 0 반납        예 : 작성자의 이름의 길이가 20바이트를 초과할 때등  -->
      <!-- sendRedirect나 forward방식에서는 script가 실행안되므로 생략함
	  <c:if test="${RRDS == 0 && isWrite == 'Yes'}">
	  
		  <script type='text/javascript'>alert('글쓰기에 실패하였습니다.\n\n다시  입력하세요.');</script> 	  
...
<input type="hidden" name="originbidx" value="0">  <!--  MaxBIDX()에서 실제값을 할당함 /IDX/ --> 
<input type="hidden" name="updown" value="0">      <!--  답글이 아닌 글은 0을 줌 --> 
<input type="hidden" name="leftright" value="0">   <!--  답글이 아닌 글은 0을 줌 --> 

<!--  
<input type="hidden" name="RRD" value="0">
-->
...
<tr>
  ...
  <td class=col2><textarea class=writer name="writer" cols = 30 rows=1 
  						   onkeyup="checkLength(this,6)">${mv.idx} / ${mv.name}</textarea>
