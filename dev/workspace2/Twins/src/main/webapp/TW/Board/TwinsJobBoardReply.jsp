...
function check() {
    ...
    form0922.action="${pageContext.request.contextPath}/twins/jbWriteActionController${pageMaker.bidxIdxTotalMakeSearch(page, jbv.jbidx, jmv.jmidx, jmv.jmname)}";
    form0922.submit();
    return;
}
...
<c:if test="${jmidx == null}">
  <c:set var="jmidx" value="${'15'}"/>	
</c:if>
<c:if test="${jmidx != null}">
  <c:set var="jmidx" value="${jmv.jmidx}"/>	
</c:if>
...
<p>scri.jbcategory = ${scri.jbcategory}</p> 
...
<!-- 
<input type="hidden" name="jmidx" value="7">
-->
...
<tr>
  <td class=bottom colspan=4>
   ...
   <input type="button" value="목록" onclick="location.href='${pageContext.request.contextPath}/twins/jbListController${pageMaker.idxMakeSearch(page, jmv.jmidx, jmv.jmname)}'">
