...
<script>
	function de(){ // function = 함수
		if(confirm("삭제 하시겠습니까?")){ // confirm = 경고창
        	location.href="<%=request.getContextPath()%>/Controller/DeleteActionServlet.do?idx=<%=idx %>";
        	}else{
        		alert("삭제되지 않았습니다.");
        	}
		}
...
<!-- %mv.getArea()% /div -->
  ...
  <a class="btn btn-default" href="<%=request.getContextPath()%>/Controller/ModifyServlet.do?idx=<%=idx %>">수정</a> <!--idx 값을 못 불러오니까 getidx였지만 설정을 안해둬서 그냥 idx 값을 불러와서 실행 -->
