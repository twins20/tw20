...
<script>
	function de(){ // function = 함수
		if(confirm("삭제 하시겠습니까?")){ // confirm = 경고창
        	location.href="<%=request.getContextPath()%>/Controller/SalaryDeleteActionServlet.do?sidx=<%=sidx %>";
        	}else{
        		alert("삭제되지 않았습니다.");
        	}
		}
...
<tr>
<td>금액</td><td>회차</td><td>인덱스</td>  <!-- 테이블 생성 -->
</tr>
<tr>
<td><%=sv.getAmount() %></td>
<td><%=sv.getRound() %></td>
...
<!-- %sv.getArea()% /div -->
  ...
  <a class="btn btn-default" href="<%=request.getContextPath()%>/Controller/SalaryModifyServlet.do?sidx=<%=sidx %>">수정</a> <!--bidx 값을 못 불러오니까 getidx였지만 설정을 안해둬서 그냥 bidx 값을 불러와서 실행 -->
