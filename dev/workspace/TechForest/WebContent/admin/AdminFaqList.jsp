...
<c:set var="pagingUrl" value="/AdminFaqList.do"/>
...
%>		
	  <div class="clearFix">
	    ...
	    <div class="col col6_2"><a href="<%=request.getContextPath()%>/AdminFaqConServlet?bIdx=<%= vo.getbIdx()%>"><%= vo.getTitle()%></a></div>
	    ...
	    <div class="col col6_6">
		    <a href="<%=request.getContextPath()%>/AdminFaqMod.do?bIdx=<%= vo.getbIdx()%>"><%= modBtn%></a>
		    <a href="<%=request.getContextPath()%>/AdminFaqDel_Action.do?bIdx=<%= vo.getbIdx()%>"><%= delBtn%></a></div>
	  </div>	  
<%
		}
%>
<!-- 페이징 처리 부분 -->
	...
	<div>
		<a href="<%=request.getContextPath()%>/AdminFaqWrite.do"><button class="btn btn-primary pull-right">글쓰기</button></a>
	</div>
</div>
