...
public class BoardQnaServiceImpl {
	...
	//QNA 카테고리 별 리스트
	public ArrayList<BoardVo> boardQnaListCate(String cate, int listCnt, int pageCnt){
	...
	//QNA 카테고리 별 리스트 Cnt
	public int boardQnaListCateTtCnt(String cate){
	...
	//QNA 전체 리스트 Cnt
	public int boardQnaListTtCnt(int idx){
	...
	//QNA 프로젝트 리스트 
	public ProjectVo boardQnaProjList(int pIdx){
	...
	public int boardQnaWrite(BoardVo vo){
		  ...
		  try{
			  ...
			  this.sql=	"INSERT INTO TF_BOARD_QNA (BIDX, IDX, CATE, TITLE, CONTENTS, HIT, GOOD, BAD, OBIDX, RBIDX, BDEPTH, COMMCNT, VIEWSTAT, INSDATE, MODDATE ) "
			  	+		"VALUES (SEQ_TF_BIDX_QNA.NEXTVAL, ?, ?, ?, ?, 0, 0, 0, SEQ_TF_BIDX_QNA.CURRVAL, 1, 1, 0, 1, SYSDATE, SYSDATE)";
	...
	//QNA 게시판 댓글 리스트
	public ArrayList<Map<String, Object>> boardQnaCommList(int bIdx, int listCnt, int pageCnt){
		...
		try{		
			...
			this.sql = "SELECT A.*, B.NICK "
				+ 	"FROM TF_BOARD_COMM_QNA A, TF_MEMBER B "
				+ 	"WHERE A.IDX = B.IDX "
				+ 		"AND A.BIDX = ? "
	...
	//QNA 게시판 댓글 리스트 Cnt
	public int boardQnaCommListTtCnt(int bIdx){
	...
	//QNA 게시판 대댓글 리스트 
	public ArrayList<Map<String, Object>> BoardQnaSubCommList(int commIdx, int bIdx, int listCnt, int pageCnt){
	...
	//QNA 게시판 대댓글 리스트 Cnt
	public int BoardQnaSubCommListTtCnt(int commIdx, int bIdx){
	...
	//QNA 게시판 댓글 작성 
	public int boardQnaCommWrite(BoardCommVo vc){
		...
		try{
			...
			this.sql = "INSERT INTO TF_BOARD_COMM_QNA (COMMIDX, BIDX, IDX, COMMENTS, GOOD, BAD, OCOMMIDX, RCOMMIDX, COMMDEPTH, VIEWSTAT, INSDATE, MODDATE ) "
				+	"VALUES (SEQ_TF_COMMIDX_QNA.NEXTVAL, ?, ?, ?, 0, 0, SEQ_TF_COMMIDX_QNA.CURRVAL, 1, 1, 1, SYSDATE, SYSDATE )";			
	...
	//QNA 대댓글 작성 
	public int boardQnaSubCommWriteTransaction(BoardCommVo vc){
		...
		try{
			...
			this.sql = "UPDATE TF_BOARD_COMM_QNA "
			...
			this.sql = "INSERT INTO TF_BOARD_COMM_QNA (COMMIDX, BIDX, IDX, COMMENTS, GOOD, BAD, OCOMMIDX, RCOMMIDX, COMMDEPTH, VIEWSTAT, INSDATE, MODDATE ) "
				+	"VALUES (SEQ_TF_COMMIDX_QNA.NEXTVAL, ?, ?, ?, 0, 0, ?, 1, 1, 1, SYSDATE, SYSDATE )";	
	...
	//QNA 댓글 수정
	public int boardQnaCommWriteCntMod(BoardCommVo vc){
	 ...
	 //QNA 댓글 삭제 
	 public int boardQnaCommDel(int commIdx){
	...
	//QNA 게시판 답글
	public int boardQnaReplyWrite(BoardVo inputBV){
		...
		try {
			...
			this.sql = "INSERT INTO TF_BOARD_QNA (BIDX, IDX, CATE, TITLE, CONTENTS, HIT, GOOD, BAD, OBIDX, RBIDX, BDEPTH, COMMCNT, VIEWSTAT, INSDATE, MODDATE, ) "
				+		"VALUES (SEQ_TF_BIDX_QNA.NEXTVAL, ?, ?, ?, ?, 0, 0, 0, (SELECT OBIDX FROM TF_BOARD_QNA WHERE BIDX = ?), (SELECT RBIDX FROM TF_BOARD_QNA WHERE BIDX = ?) + 1, (SELECT BDEPTH FROM TF_BOARD_QNA WHERE BIDX = ?) + 1, 0, 1, SYSDATE, SYSDATE) ";
